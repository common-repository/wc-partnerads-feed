    ...
    public function render() {
        ...
        while ( $feed_query->have_posts() ) {
			...
			$item->addChild( 'kategorinavn', '')->addCData($default_category);
			 ...
			 $brand = sanitize_text_field( @$options['brand_name'] );
            ...
            if($brand == '') {
               ...
               $item->addChild( 'brand', '')->addCData($brand);
            ...
            if ( $thumb && $settings['allow_images_in_feed'] == 'yes' ) {
                ...
                if (!preg_match("~^(?:f|ht)tps?://~i", $url)) {
                    ...
                    $url = "".get_site_url()."" . $image_url[0];

                }

                $item->addChild( 'billedurl',  htmlspecialchars($url));
            }
